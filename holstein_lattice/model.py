...
def initialize(sim):
    ...
    def hop(z, zc, delta_z, delta_zc, ev_diff): # this is the hoping function for a harmonic oscillator
        hopped = False
        akj_z = np.real(np.sum(sim.w * delta_zc * delta_z))
        bkj_z = np.real(np.sum(1j * sim.w * (zc * delta_z - z * delta_zc)))
        ckj_z = ev_diff
