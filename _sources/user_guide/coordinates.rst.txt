...
Users may notice that :math:`h_{n}` plays the same role as a frequency in relating the relative displacements of position and momentum.
...
When implementing a new gradient ingredient (such as ``dh_qc_dzc`` or
``dh_c_dzc``), it is often natural to differentiate the underlying
physics with respect to :math:`q` and :math:`p`, and then convert the
result to the gradient with respect to :math:`z^{*}` that QC Lab uses
internally. The reverse direction is occasionally needed as well, for
example when comparing to phase-space gradients reported by an
electronic-structure code. Two helpers are provided for these
conversions:
...
The Tully Model objects (``TullyProblemOne``, ``TullyProblemTwo``,
``TullyProblemThree``) and the ``AbInitio`` Model object use
``dqdp_to_dzc`` to assemble their analytical gradient ingredients. See
the :ref:`Functions <functions>` section for the full list of
low-level helpers.
