...
class ExtendedCoupling(Model):
    ...
    def dh_qc_dzc(self, parameters, **kwargs):
        ...
        indices_pos = (np.real(z) >= 0)
        dV_12_dzc[indices_pos] = (B*C*gradient_weight)*(np.exp(-1*C*gradient_weight*(z[indices_pos] + np.conj(z[indices_pos]))))
        indices_neg = (np.real(z) < 0)
        dV_12_dzc[indices_neg] = (B*C*gradient_weight)*(np.exp(C*gradient_weight*(z[indices_neg] + np.conj(z[indices_neg]))))
