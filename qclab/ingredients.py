...
def update_h_quantum(sim, state):
    state.h_quantum = np.zeros((sim.num_branches * sim.num_trajs, sim.num_states, sim.num_states), dtype=complex) + sim.h_q(sim.h_q_params)[np.newaxis, :, :] + \
...
def update_quantum_force_wf_db(sim, state):
    ...
    auxilliary.quantum_force_branch(state.wf_db, None, state.z_coord, sim)
    return state


def update_quantum_force_act_surf(sim, state):
    state.quantum_force = np.zeros((sim.num_branches * sim.num_trajs, sim.num_classical_coordinates), dtype=complex) + \
    auxilliary.quantum_force_branch(state.eigvecs, state.act_surf_ind, state.z_coord, sim)
    return state


def update_z_coord_RK4(sim, state):
    state.z_coord = auxilliary.rk4_c(state.z_coord, state.quantum_force, sim.dt, sim)
    return state

def update_wf_db_RK4(sim, state):
    # evolve wf_db using an RK4 solver
    state.wf_db = auxilliary.rk4_q_branch(state.h_quantum, state.wf_db, sim.dt)
    return state
...
def update_branch_phase(sim, state):
    state.branch_phase = state.branch_phase + sim.dt * state.eigvals[np.arange(sim.num_branches*sim.num_trajs, dtype=int), state.act_surf_ind_0]
    return state
...
def initialize_wf_adb_delta(sim, state):
    ...
    for nt in range(sim.num_trajs):
        wf_adb_delta[nt][np.arange(sim.num_branches, dtype=int), state.act_surf_ind_0[nt * sim.num_branches:(nt + 1) * sim.num_branches]] = 1.0 + 0.j
    state.wf_adb_delta = wf_adb_delta.reshape((sim.num_trajs * sim.num_branches, sim.num_states))
    # transform to diabatic basis
    state.wf_db_delta = auxilliary.psi_adb_to_db_branch(state.wf_adb_delta, state.eigvecs)
    return state

def update_wf_db_eigs(sim, state):
    state.wf_db, state.wf_adb = auxilliary.evolve_wf_eigs(state.wf_db, state.eigvals, state.eigvecs, sim.dt)
    return state

def update_wf_db_delta_eigs(sim, state):
    state.wf_db_delta, state.wf_adb_delta = auxilliary.evolve_wf_eigs(state.wf_db_delta, state.eigvals, state.eigvecs, sim.dt)
    return state
...
def initialize_wf_adb(sim, state):
    state.wf_adb = auxilliary.psi_db_to_adb_branch(state.wf_db, state.eigvecs)
    return state 
...
def initialize_active_surface(sim, state):
    ...
    if sim.sh_deterministic:
        ...
        act_surf_ind_0 = np.zeros((sim.num_trajs, sim.num_branches)) + np.arange(sim.num_branches, dtype=int)[np.newaxis, :]
    else:
...
def analytic_gauge_fix_eigs(sim, state):
    ...
    for n in range(sim.num_trajs*sim.num_branches):
        ...
        dab_q_phase, dab_p_phase = auxilliary.get_dab_phase(state.eigvals[n], state.eigvecs[n], state.z_coord[n], sim)
        # execute phase shift
...
def gauge_fix_eigs(sim, state):
    for nt in range(sim.num_trajs):
        ...
        state.eigvecs[nt * sim.num_branches:(nt + 1) * sim.num_branches], _ = auxilliary.sign_adjust_branch(
            ...
            for k in range(len(hop_prob)):
                ...
                if rand < bin_edge:
                    ...
                    dkj_z, dkj_zc = auxilliary.get_dab(evec_k, evec_j, ev_diff,
                                                        state.z_coord[nt * sim.num_branches:(nt + 1) * sim.num_branches][i],
                                                        sim)
                    ...
                    state.z_coord[nt * sim.num_branches:(nt + 1) * sim.num_branches][i], hopped = \
                        sim.hop(sim, state.z_coord[nt * sim.num_branches:(nt + 1) * sim.num_branches][i], delta_z, ev_diff)
...
def update_dm_db_FSSH(sim, state):
    state.dm_db_branch = auxilliary.rho_adb_to_db_branch(state.dm_adb_branch, state.eigvecs)
    if sim.sh_deterministic:
        state.dm_db_branch = ((np.einsum('njj->nj', state.dm_adb_0).reshape((sim.num_trajs, sim.num_states)))[:, :,
                np.newaxis, np.newaxis] * state.dm_db_branch.reshape(sim.num_trajs, sim.num_branches, sim.num_states, sim.num_states))
    else:
...
def update_e_q_MF(sim, state):
    state.e_q_branch = np.sum(np.einsum('nj,nji,ni->n', np.conj(state.wf_db), state.h_quantum, state.wf_db).reshape((sim.num_trajs, sim.num_branches)),axis=0)
...
def update_branch_pair_eigs(sim, state):
    state.eigvals_branch_pair = np.zeros((sim.num_trajs, sim.num_branches, sim.num_branches, sim.num_states), dtype=float)
    state.eigvecs_branch_pair = np.zeros((sim.num_trajs, sim.num_branches, sim.num_branches, sim.num_states, sim.num_states), dtype=complex)
    for nt in range(sim.num_trajs):
        state.eigvals_branch_pair[nt], state.eigvecs_branch_pair[nt] = auxilliary.get_branch_pair_eigs(
            state.z_coord[nt * sim.num_branches:(nt + 1) * sim.num_branches], sim)
    return state

def gauge_fix_branch_pair_eigs(sim, state):
    for nt in range(sim.num_trajs):
        state.eigvals_branch_pair[nt], state.eigvecs_branch_pair[nt] = auxilliary.sign_adjust_branch_pair_eigs(state.z_coord[nt * sim.num_branches:(nt + 1) * sim.num_branches], \
