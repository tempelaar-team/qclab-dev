...
def initialize_wf_db(state):
    state.wf_db = np.zeros((state.model.batch_size, state.model.num_branches, state.model.num_states), dtype=complex) + state.model.wf_db[
                                                                                                     np.newaxis, :]
    return state


def initialize_z_coord(state):
    state.z_coord = np.zeros((state.model.batch_size, state.model.num_branches, state.model.num_classical_coordinates), dtype=complex)
...
def update_e_q_mf(state):
    state.e_q = np.einsum('tbj,tbji,tbi', np.conj(state.wf_db), state.h_quantum, state.wf_db, optimize='greedy')
    return state
...
def analytic_gauge_fix_eigs(state):
    ...
    for traj_n in range(state.model.batch_size):
        for branch_n in range(state.model.num_branches):
            ...
            der_couple_q_phase, der_couple_p_phase = auxiliary.get_der_couple_phase(state, state.eigvals[traj_n, branch_n],
                                                               state.eigvecs[traj_n, branch_n])
            # execute phase shift
...
def gauge_fix_eigs(state):
    for nt in range(state.model.batch_size):  # TODO can this loop be eliminated? Make sign_adjust_branch not branch dependent?
        ...
        state.eigvecs[nt], _ = auxiliary.sign_adjust_branch(state.eigvecs[nt], state.eigvecs_previous[nt],
                                                            state.eigvals[nt], state.z_coord[nt], model)
    return state
...
def update_active_surface_fssh(state):
    ...
    for nt in range(state.model.batch_size):
        ...
        for i in range(state.model.num_branches):
            ...
            prod = np.matmul(np.conjugate(state.eigvecs[nt, i][:,
                                          state.act_surf_ind[nt, i]]),
            ...
            for k in range(len(hop_prob)):
                ...
                if rand < bin_edge:
                    ...
                    dkj_z, dkj_zc = auxiliary.get_der_couple(state, evec_k, evec_j, ev_diff)
                    ...
                    delta_z = dkj_zc
                    state.z_coord[nt, i], hopped = state.model.hop(state.z_coord[nt, i], delta_z, ev_diff)
...
def update_dm_db_fssh(state):
    ...
    if state.model.sh_deterministic:
        state.dm_adb = np.einsum('tbb->tb', state.dm_adb_0, optimize='greedy')[:, :, np.newaxis,
                       np.newaxis] * state.dm_adb
    else:
...
def initialize_wf_adb_delta(state):
    ...
    for nt in range(state.model.batch_size):
        wf_adb_delta[nt][np.arange(state.model.num_branches, dtype=int), state.act_surf_ind_0[nt * state.model.num_branches:(
                                                                                                                            nt + 1) * state.model.num_branches]] = 1.0 + 0.j
    ...
    for nt in range(state.model.batch_size):
        ...
        for i in range(state.model.num_branches):
            ...
            prod = np.matmul(np.conjugate(state.eigvecs[nt, i][:, state.act_surf_ind[nt, i]]),
                             state.eigvecs_previous[nt, i])
            hop_prob = -2 * np.real(
                prod * (state.wf_adb_delta[nt, i] / state.wf_adb_delta[nt, i][state.act_surf_ind[nt, i]]))
            hop_prob[state.act_surf_ind[nt, i]] = 0
            bin_edge = 0
            # hop if possible
            for k in range(len(hop_prob)):
...
def update_classical_overlap(state):
    ...
    for nt in range(state.model.batch_size):
        state.overlap[nt] = auxiliary.get_classical_overlap(
            state.z_coord[nt * state.model.num_branches:(nt + 1) * state.model.num_branches], model)
    return state
...
def update_dm_db_cfssh(state):
    ...
    for nt in range(state.model.batch_size):
        ...
        branch_phase_nt = state.branch_phase[nt * state.model.num_branches:(nt + 1) * state.model.num_branches]
        act_surf_ind_nt = state.act_surf_ind[nt * state.model.num_branches:(nt + 1) * state.model.num_branches]
        act_surf_nt = state.act_surf[nt * state.model.num_branches:(nt + 1) * state.model.num_branches]
        act_surf_ind_0_nt = state.act_surf_ind_0[nt * state.model.num_branches:(nt + 1) * state.model.num_branches]
        z_coord_nt = state.z_coord[nt * state.model.num_branches:(nt + 1) * state.model.num_branches]
        for i in range(state.model.num_branches):
    ...
    state.dm_adb = state.dm_adb.reshape((state.model.batch_size * state.model.num_branches, state.model.num_states, state.model.num_states))
    state.dm_db = auxiliary.rho_adb_to_db_branch(state.dm_adb, state.eigvecs).reshape(
        (state.model.batch_size, state.model.num_branches, state.model.num_states, state.model.num_states))
    if state.model.sh_deterministic:
...
def update_branch_pair_eigs(state):
    state.eigvals_branch_pair = np.zeros((state.model.batch_size, state.model.num_branches, state.model.num_branches, state.model.num_states),
                                         dtype=float)
    state.eigvecs_branch_pair = np.zeros(
        (state.model.batch_size, state.model.num_branches, state.model.num_branches, state.model.num_states, state.model.num_states), dtype=complex)
    for nt in range(state.model.batch_size):
...
def gauge_fix_branch_pair_eigs(state):
    for nt in range(state.model.batch_size):
        state.eigvals_branch_pair[nt], state.eigvecs_branch_pair[nt] = auxiliary.sign_adjust_branch_pair_eigs(
            state.z_coord[nt * state.model.num_branches:(nt + 1) * state.model.num_branches],
            state.eigvecs_branch_pair[nt], state.eigvals_branch_pair[nt], state.eigvecs_branch_pair_previous[nt], model)
    return state
...
def analytic_gauge_fix_branch_pair_eigs(state):
    # compute initial eigenvalues and eigenvectors in each branch
    for n in range(state.model.batch_size * state.model.num_branches):
        # compute initial gauge shift for real-valued derivative couplings
        der_couple_q_phase, der_couple_p_phase = auxiliary.get_der_couple_phase(state.eigvals[n], state.eigvecs[n], state.z_coord[n], model)
        # execute phase shift
        state.eigvecs[n] = np.copy(np.matmul(state.eigvecs[n], np.diag(np.conjugate(der_couple_q_phase))))
        # recalculate phases and check that they are zero
        der_couple_q_phase, der_couple_p_phase = auxiliary.get_der_couple_phase(state.eigvals[n], state.eigvecs[n], state.z_coord[n], model)
        if np.sum(np.abs(np.imag(der_couple_q_phase)) ** 2 + np.abs(np.imag(der_couple_p_phase)) ** 2) > 1e-10:
            # this error will indicate that symmetries of the Hamiltonian have been broken by the representation
