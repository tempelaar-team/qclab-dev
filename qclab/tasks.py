...
def initialize_z_coord(sim, parameters, state, **kwargs):
    ...
    if hasattr(sim.model, "init_classical"):
        state.modify(
            "z_coord",
            sim.model.init_classical(sim.model.constants, parameters, seed=seed),
        )
    else:
        warnings.warn(
            "model.init_classical not specified, using numerical boltzmann.",
            UserWarning,
        )
        state.modify(
            "z_coord",
            ingredients.numerical_boltzmann_init_classical(sim.model, seed=state.seed),
        )
...
def update_dh_c_dzc(sim, parameters, state, **kwargs):
    ...
    if hasattr(sim.model, "dh_c_dzc"):
        ...
        state.modify(
            "dh_c_dzc",
            sim.model.dh_c_dzc(sim.model.constants, parameters, z_coord=z_coord),
        )
    else:
        ...
        h_c_0 = sim.model.h_c(sim.model.constants, parameters, z_coord=z_coord)
        dh_c_dzc = np.zeros((len(z_coord), *np.shape(h_c_0)), dtype=complex)

        for n in range(len(z_coord)):
            h_c_offset_re = sim.model.h_c(
                sim.model.constants, parameters, z_coord=offset_z_coord_re[n]
            )
            diff_re = (h_c_offset_re - h_c_0) / delta_z
            h_c_offset_im = sim.model.h_c(
                sim.model.constants, parameters, z_coord=offset_z_coord_im[n]
            )
...
def update_dh_c_dzc_vectorized(sim, parameters, state, **kwargs):
    ...
    if hasattr(sim.model, "dh_c_dzc_vectorized"):
        ...
        if hasattr(sim.model, "dh_c_dzc"):
            state.modify(
                "dh_c_dzc",
                apply_non_vectorized_ingredient(
                    sim,
                    sim.model.dh_c_dzc,
                    sim.model.constants,
                    parameters,
...
def update_dh_qc_dzc_vectorized(sim, parameters, state, **kwargs):
    ...
    if hasattr(sim.model, "dh_qc_dzc_vectorized"):
        state.modify(
            "dh_qc_dzc",
            sim.model.dh_qc_dzc_vectorized(
                sim.model.constants, parameters, z_coord=z_coord
            ),
        )
    else:
...
def update_h_quantum_vectorized(sim, parameters, state, **kwargs):
    ...
    if hasattr(sim.model, "h_q_vectorized"):
        ...
        h_q = np.array(
            [
                sim.model.h_q(sim.model.constants, parameters._element_list[n])
                for n in range(sim.settings.batch_size)
            ]
        )
...
def update_classical_energy_vectorized(sim, parameters, state, **kwargs):
    ...
    if hasattr(sim.model, "h_c_vectorized"):
        state.modify(
            "classical_energy",
            sim.model.h_c_vectorized(sim.model.constants, parameters, z_coord=z_coord),
        )
    else:
        warnings.warn(
...
def update_classical_energy_fssh_vectorized(sim, state, **kwargs):
    ...
    if hasattr(sim.model, "h_c_vectorized"):
        state.modify(
            "classical_energy",
            np.sum(sim.model.h_c_vectorized(z_coord=z_coord), axis=-1),
        )
    else:
        state.modify(
            "classical_energy",
...
def update_quantum_energy_mf(sim, state, **kwargs):
    ...
    wf = kwargs["wf"]
    state.modify(
        "quantum_energy", np.matmul(np.conj(wf), np.matmul(state.h_quantum, wf))
    )
    return state
...
def broadcast_var_to_branch_vectorized(sim, state, **kwargs):
    ...
    val = kwargs["val"]
    out = (
        np.zeros((len(val), sim.algorithm.parameters.num_branches, *np.shape(val)[1:]))
        + val[:, np.newaxis, ...]
    )
...
def initialize_active_surface(sim, state, **kwargs):
    ...
    act_surf[
        np.arange(sim.algorithm.parameters.num_branches, dtype=int), state.act_surf_ind
    ] = 1
    state.modify(
        "act_surf",
...
def initialize_random_values_fssh(sim, state, **kwargs):
    ...
    state.modify("hopping_probs_rand_vals", np.random.rand(len(sim.settings.tdat)))
    state.modify(
        "stochastic_sh_rand_vals", np.random.rand(sim.algorithm.parameters.num_branches)
    )
    return state
        ...
        for k in range(len(hop_prob)):
            ...
            if rand < bin_edge:
                ...
                if hasattr(sim.model, "hop_function"):
                    ...
                    z_coord_branch_i, hopped = ingredients.numerical_hop(
                        z_coord=z_coord_branch[i],
                        delta_z_coord=delta_z,
                        ev_diff=ev_diff,
                    )
