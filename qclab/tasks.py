...
def vector_apply_all_but_last(func, arg):
    ...
    """
    arg_vec = arg.reshape(
        (np.prod(np.shape(arg)[:-1]), np.shape(arg)[-1]))  # Reshape to 2D array
    val_vec = np.array([func(arg_vec[n])
                       for n in range(len(arg_vec))])  # Vectorize function
...
def initialize_z_coord(sim, state, **kwargs):
    ...
    if hasattr(sim.model, 'init_classical'):
        ...
        state.modify('z_coord', ingredients.numerical_boltzmann_init_classical(sim.model, seed=state.seed))
    return state
...
def dh_c_dzc_finite_differences(sim, state, **kwargs):
    ...
    offset_z_coord_im = z_coord[np.newaxis, :] + \
        1j * np.identity(len(z_coord)) * delta_z
...
def update_dh_c_dzc_vectorized(sim, state, **kwargs):
    ...
    if hasattr(sim.model, 'dh_c_dzc_vectorized'):
        ...
        warnings.warn(
            "dh_c_dzc_vectorized not implemented for this model. Using non-vectorized method.", UserWarning)
        if hasattr(sim.model, 'dh_c_dzc'):
            state.modify('dh_c_dzc', vector_apply_all_but_last(
                lambda z: sim.model.dh_c_dzc(z_coord=z), z_coord))
        else:
            warnings.warn(
                "dh_c_dzc not implemented for this model. Using finite differences.", UserWarning)
            state.modify('dh_c_dzc', vector_apply_all_but_last(
                lambda z: dh_c_dzc_finite_differences(sim, state, z_coord=z), z_coord))
...
def update_dh_qc_dzc_vectorized(sim, state, **kwargs):
    ...
    if hasattr(sim.model, 'dh_qc_dzc_vectorized'):
        ...
        warnings.warn(
            "dh_qc_dzc_vectorized not implemented for this model. Using non-vectorized method.", UserWarning)
        if hasattr(sim.model, 'dh_qc_dzc'):
            state.modify('dh_qc_dzc', vector_apply_all_but_last(
                lambda z: sim.model.dh_qc_dzc(z_coord=z), z_coord))
        else:
            warnings.warn(
                "dh_qc_dzc not implemented for this model. Using finite differences.", UserWarning)
            state.modify('dh_qc_dzc', vector_apply_all_but_last(
                lambda z: dh_qc_dzc_finite_differences(sim, state, z_coord=z), z_coord))
...
def update_quantum_classical_forces(sim, state, **kwargs):
    ...
    state.modify('quantum_classical_forces', np.dot(
...
def update_h_quantum_vectorized(sim, state, **kwargs):
    ...
    if hasattr(sim.model, 'h_q_vectorized'):
        ...
        warnings.warn(
            "h_q_vectorized not implemented for this model. Using non-vectorized method.", UserWarning)
        h_q = sim.model.h_q()
    if hasattr(sim.model, 'h_qc_vectorized'):
        h_tot = h_q + sim.model.h_qc_vectorized(z_coord=z_coord) + 0.0j
    else:
        warnings.warn(
            "h_qc_vectorized not implemented for this model. Using non-vectorized method.", UserWarning)
        h_tot = np.array([(h_q + sim.model.h_qc(z_coord=z_coord[n]) + 0j)
                         for n in range(len(z_coord))])
...
def broadcast_var_to_branch_vectorized(sim, state, **kwargs):
    name = kwargs['name']
    val = kwargs['val']
    out = np.zeros((len(val), sim.algorithm.parameters.num_branches,
                   *np.shape(val)[1:])) + val[:, np.newaxis, ...]
...
def gauge_fix_eigs_vectorized(sim, state, **kwargs):
    ...
    if kwargs['gauge_fixing'] >= 1:
        phase = np.exp(-1.0j *
                       np.angle(np.sum(np.conj(eigvecs_previous) * eigvecs, axis=-2)))
...
def basis_transform_vec_vectorized(sim, state, **kwargs):
    ...
    basis = kwargs['basis']
    output_name = kwargs['output_name']
    state.modify(output_name, np.einsum(
        '...ij,...j->...i', basis, input_vec, optimize='greedy'))
    return state
...
def initialize_active_surface(sim, state, **kwargs):
    ...
    if sim.algorithm.parameters.fssh_deterministic:
        ...
        for branch_n in range(sim.algorithm.parameters.num_branches):
            act_surf_ind_0[branch_n] = \
                np.arange(num_states, dtype=int)[
                intervals > state.stochastic_sh_rand_vals[branch_n]][0]
    ...
    act_surf[np.arange(sim.algorithm.parameters.num_branches,
                       dtype=int), state.act_surf_ind] = 1
    state.modify('act_surf', act_surf.reshape(
        (sim.algorithm.parameters.num_branches, num_states)).astype(int))
    return state
...
def initialize_random_values_fssh(sim, state, **kwargs):
    ...
    state.modify('stochastic_sh_rand_vals', np.random.rand(
...
def initialize_dm_adb_0_fssh_vectorized(sim, state, **kwargs):
    del sim, kwargs
    state.modify('dm_adb_0', np.einsum('...i,...j->...ij',
                 state.wf_adb_branch, np.conj(state.wf_adb_branch)) + 0.0j)
    return state
...
def update_dm_db_fssh_vectorized(sim, state, **kwargs):
    del kwargs
    dm_adb_branch = np.einsum('...i,...j->...ij', state.wf_adb_branch,
                              np.conj(state.wf_adb_branch), optimize='greedy')
    for nt in range(len(dm_adb_branch)):
        np.einsum('...jj->...j', dm_adb_branch[nt])[...] = state.act_surf[nt]
    if sim.algorithm.parameters.fssh_deterministic:
        dm_adb_branch = np.einsum(
            'tbb->tb', state.dm_adb_0[..., 0, :, :])[..., np.newaxis, np.newaxis] * dm_adb_branch
    else:
    ...
    basis_transform_mat_vectorized(
        sim, state, input_mat=dm_adb_branch, basis=state.eigvecs, output_name='dm_db_branch')
...
def update_active_surface_fssh(sim, state, **kwargs):
    ...
    for i in range(sim.algorithm.parameters.num_branches):
        ...
        prod = np.matmul(
            np.conj(state.eigvecs[i][:, state.act_surf_ind[i]]), state.eigvecs_previous[i])
        hop_prob = -2 * \
            np.real(
                prod * (state.wf_adb_branch[i] / (state.wf_adb_branch[i][state.act_surf_ind[i]])))
        ...
        for k in range(len(hop_prob)):
            ...
            if rand < bin_edge:
                ...
                dkj_zc = np.einsum('...i,...nij,...j->...n', np.conj(evec_k), state.dh_qc_dzc[i], evec_j) / ev_diff[
                    ..., np.newaxis]
                dkj_p = np.sqrt(1 / (2 * sim.model.parameters.pq_weight *
                                sim.model.parameters.mass)) * (dkj_z - dkj_zc)
                dkj_q = np.sqrt(sim.model.parameters.pq_weight *
                                sim.model.parameters.mass / 2) * (dkj_z + dkj_zc)
                ...
                if np.abs(dkj_q[max_pos_q]) > 1e-8 and np.abs(np.sin(np.angle(dkj_q[max_pos_q]))) > 1e-2:
                    warnings.warn(
                        'dkj_q Nonadiabatic coupling is complex, needs gauge fixing!', UserWarning)
                if np.abs(dkj_p[max_pos_p]) > 1e-8 and np.abs(np.sin(np.angle(dkj_p[max_pos_p]))) > 1e-2:
                    warnings.warn(
                        'dkj_p Nonadiabatic coupling is complex, needs gauge fixing!', UserWarning)
