...
def vector_apply_all_but_last(func, arg):
    ...
    Parameters:
    func (callable): The function to be vectorized. It should take a 1D array 
    as input and return an array.
    arg (ndarray): The input array to be processed. The function will be applied 
    to all slices along the last axis.

    Returns:
    ndarray: The output array with the function applied to each slice along the 
    last axis of the input array.
    """
    arg_vec = arg.reshape(
        (np.prod(np.shape(arg)[:-1]), np.shape(arg)[-1])
    )  # Reshape to 2D array
    val_vec = np.array(
        [func(arg_vec[n]) for n in range(len(arg_vec))]
    )  # Vectorize function
...
def initialize_z_coord(sim, state, **kwargs):
    ...
    if hasattr(sim.model, "init_classical"):
        ...
        state.modify(
            "z_coord",
            ingredients.harmonic_oscillator_boltzmann_init_classical(
...
def dh_c_dzc_finite_differences(sim, state, **kwargs):
    """
    Calculates gradient of classical Hamiltonian with respect 
    to z-coordinates using finite differences.
    """
    del state
    z_coord = kwargs["z_coord"]
    # Approximate the gradient using finite differences
    delta_z = 1e-3
    offset_z_coord_re = z_coord[np.newaxis, :] + \
        np.identity(len(z_coord)) * delta_z
    offset_z_coord_im = (
...
def dh_qc_dzc_finite_differences(sim, state, **kwargs):
    """
    Calculates gradient of quantum-classical Hamiltonian with respect 
    to z-coordinates using finite differences.
    """
    del state
    z_coord = kwargs["z_coord"]
...
def update_dh_c_dzc_vectorized(sim, state, **kwargs):
    ...
    if hasattr(sim.model, "dh_c_dzc_vectorized"):
        state.modify(
            "dh_c_dzc", sim.model.dh_c_dzc_vectorized(z_coord=z_coord))
    else:
        warnings.warn(
            "dh_c_dzc_vectorized not implemented for this model. Using non-vectorized method.",
            UserWarning,
        )
        if hasattr(sim.model, "dh_c_dzc"):
            state.modify(
                "dh_c_dzc",
                vector_apply_all_but_last(
                    lambda z: sim.model.dh_c_dzc(z_coord=z), z_coord
                ),
            )
        else:
            warnings.warn(
                "dh_c_dzc not implemented for this model. Using finite differences.",
                UserWarning,
            )
            state.modify(
                "dh_c_dzc",
                vector_apply_all_but_last(
                    lambda z: dh_c_dzc_finite_differences(
                        sim, state, z_coord=z),
                    z_coord,
                ),
            )
...
def update_dh_qc_dzc_vectorized(sim, state, **kwargs):
    """
    Update the gradient of the quantum-classical Hamiltonian with 
    respect to the z-coordinates (vectorized).
    ...
    if hasattr(sim.model, "dh_qc_dzc_vectorized"):
        state.modify(
            "dh_qc_dzc", sim.model.dh_qc_dzc_vectorized(z_coord=z_coord))
    else:
        warnings.warn(
            "dh_qc_dzc_vectorized not implemented for this model. Using non-vectorized method.",
            UserWarning,
        )
        if hasattr(sim.model, "dh_qc_dzc"):
            state.modify(
                "dh_qc_dzc",
                vector_apply_all_but_last(
                    lambda z: sim.model.dh_qc_dzc(z_coord=z), z_coord
                ),
            )
        else:
            warnings.warn(
                "dh_qc_dzc not implemented for this model. Using finite differences.",
                UserWarning,
            )
            state.modify(
                "dh_qc_dzc",
                vector_apply_all_but_last(
                    lambda z: dh_qc_dzc_finite_differences(
                        sim, state, z_coord=z),
                    z_coord,
                ),
            )
...
def update_quantum_classical_forces(sim, state, **kwargs):
    ...
    state.modify(
        "quantum_classical_forces", np.dot(
            np.dot(state.dh_qc_dzc, wf), np.conj(wf))
    )
    return state
...
def update_z_coord_rk4(sim, state, **kwargs):
    ...
    state = update_quantum_classical_forces(
        sim, state, wf=wf, z_coord=state.z_coord)
    k1 = -1.0j * (state.classical_forces + state.quantum_classical_forces)
    state = update_classical_forces(
        sim, state, z_coord=state.z_coord + 0.5 * dt * k1)
    if update_quantum_classical_forces_bool:
        state = update_quantum_classical_forces(
            sim, state, wf=wf, z_coord=state.z_coord + 0.5 * dt * k1
        )
    k2 = -1.0j * (state.classical_forces + state.quantum_classical_forces)
    state = update_classical_forces(
        sim, state, z_coord=state.z_coord + 0.5 * dt * k2)
    if update_quantum_classical_forces_bool:
        state = update_quantum_classical_forces(
            sim, state, wf=wf, z_coord=state.z_coord + 0.5 * dt * k2
        )
    k3 = -1.0j * (state.classical_forces + state.quantum_classical_forces)
    state = update_classical_forces(
        sim, state, z_coord=state.z_coord + dt * k3)
    if update_quantum_classical_forces_bool:
        state = update_quantum_classical_forces(
            sim, state, wf=wf, z_coord=state.z_coord + dt * k3
        )
    k4 = -1.0j * (state.classical_forces + state.quantum_classical_forces)
    state.modify("z_coord", state.z_coord + dt *
...
def update_z_coord_rk4_vectorized(sim, state, **kwargs):
    ...
    state = update_quantum_classical_forces_vectorized(
        sim, state, wf=wf, z_coord=z_coord_0
    )
    k1 = -1.0j * (state.classical_forces + state.quantum_classical_forces)
    state = update_classical_forces_vectorized(
        sim, state, z_coord=z_coord_0 + 0.5 * dt * k1
    )
    if update_quantum_classical_forces_bool:
        state = update_quantum_classical_forces_vectorized(
            sim, state, wf=wf, z_coord=z_coord_0 + 0.5 * dt * k1
        )
    k2 = -1.0j * (state.classical_forces + state.quantum_classical_forces)
    state = update_classical_forces_vectorized(
        sim, state, z_coord=z_coord_0 + 0.5 * dt * k2
    )
    if update_quantum_classical_forces_bool:
        state = update_quantum_classical_forces_vectorized(
            sim, state, wf=wf, z_coord=z_coord_0 + 0.5 * dt * k2
        )
    k3 = -1.0j * (state.classical_forces + state.quantum_classical_forces)
    state = update_classical_forces_vectorized(
        sim, state, z_coord=z_coord_0 + dt * k3)
    if update_quantum_classical_forces_bool:
        state = update_quantum_classical_forces_vectorized(
            sim, state, wf=wf, z_coord=z_coord_0 + dt * k3
        )
    k4 = -1.0j * (state.classical_forces + state.quantum_classical_forces)
    state.modify(output_name, z_coord_0 + dt *
                 0.166667 * (k1 + 2 * k2 + 2 * k3 + k4))
    return state
...
def update_h_quantum(sim, state, **kwargs):
    ...
    """
    z_coord = kwargs["z_coord"]
    state.modify("h_quantum", sim.model.h_q() +
                 sim.model.h_qc(z_coord=z_coord) + 0j)
    return state
...
def update_h_quantum_vectorized(sim, state, **kwargs):
    ...
    if hasattr(sim.model, "h_qc_vectorized"):
        ...
        h_tot = np.array(
            [
                (h_q + sim.model.h_qc(z_coord=z_coord[n]) + 0j)
                for n in range(len(z_coord))
            ]
        )
...
def update_wf_db_rk4(sim, state, **kwargs):
    ...
    state.modify("wf_db", state.wf_db + dt *
                 0.166667 * (k1 + 2 * k2 + 2 * k3 + k4))
    return state
...
def update_classical_energy_vectorized(sim, state, **kwargs):
    ...
    if hasattr(sim.model, "h_c_vectorized"):
        state.modify(
            "classical_energy", sim.model.h_c_vectorized(z_coord=z_coord)[
                :, np.newaxis]
        )
    else:
        warnings.warn(
        ...
        state.modify(
            "classical_energy",
            vector_apply_all_but_last(lambda z: sim.model.h_c(z_coord=z), z_coord)[
                :, np.newaxis
            ],
        )
    return state
...
def update_classical_energy_fssh_vectorized(sim, state, **kwargs):
    ...
    if hasattr(sim.model, "h_c_vectorized"):
        state.modify(
            "classical_energy",
            np.sum(sim.model.h_c_vectorized(
                z_coord=z_coord), axis=-1)[:, np.newaxis],
        )
    else:
        state.modify(
            "classical_energy",
...
def update_quantum_energy_mf(sim, state, **kwargs):
    ...
    state.modify(
        "quantum_energy",
        np.matmul(np.conj(wf), np.matmul(state.h_quantum, wf))[np.newaxis],
    )
    return state
...
def update_quantum_energy_mf_vectorized(sim, state, **kwargs):
    ...
    state.modify(
        "quantum_energy",
        np.einsum("...i,...ij,...j->...", np.conj(wf), state.h_quantum, wf)[
            :, np.newaxis
        ],
    )
    return state
...
def update_quantum_energy_fssh_vectorized(sim, state, **kwargs):
    """
    Calculate the quantum energy for surface hopping
    """
    del sim, kwargs
    state.modify(
        "quantum_energy",
...
def broadcast_var_to_branch_vectorized(sim, state, **kwargs):
    """
    Broadcast a variable to each branch of the wavefunction.
    """
    name = kwargs["name"]
    val = kwargs["val"]
    out = (
        np.zeros(
            (len(val), sim.algorithm.parameters.num_branches, *np.shape(val)[1:]))
        + val[:, np.newaxis, ...]
    )
...
def diagonalize_matrix_vectorized(sim, state, **kwargs):
    """ 
    Diagonalize a given matrix and store the eigenvalues and eigenvectors.
    """
    del sim
...
def analytic_der_couple_phase(sim, state, eigvals, eigvecs):
    """
    Calculate the complex phase factors needed 
    to make the derivative coupligns real-valued if possible.
    """
...
def gauge_fix_eigs_vectorized(sim, state, **kwargs):
    """
    Apply gauge fixing to the eigenvectors.
    """
...
def copy_value_vectorized(sim, state, **kwargs):
    """ 
    copy a value in the state object
    """
    del sim
    name = kwargs["name"]
    val = kwargs["val"]
...
def basis_transform_vec_vectorized(sim, state, **kwargs):
    """
    Transform the basis of a vectorized vector.
    """
    del sim
    ...
    output_name = kwargs["output_name"]
    state.modify(
        output_name, np.einsum("...ij,...j->...i", basis,
                               input_vec, optimize="greedy")
    )
    return state
...
def basis_transform_mat_vectorized(sim, state, **kwargs):
    """
    Transform the basis of a vectorized matrix.
    """
    del sim
    # default is adb to db
...
def initialize_active_surface(sim, state, **kwargs):
    """
    Initialize the active surfaces in FSSH
    """
    del kwargs
    ...
    if sim.algorithm.parameters.fssh_deterministic:
        ...
        act_surf_ind_0 = np.arange(
            sim.algorithm.parameters.num_branches, dtype=int)
    else:
    ...
    act_surf = np.zeros(
        (sim.algorithm.parameters.num_branches, num_states), dtype=int)
    act_surf[
        np.arange(sim.algorithm.parameters.num_branches,
                  dtype=int), state.act_surf_ind
    ] = 1
    state.modify(
        "act_surf",
        act_surf.reshape((sim.algorithm.parameters.num_branches, num_states)).astype(
            int
        ),
    )
    return state
...
def initialize_random_values_fssh(sim, state, **kwargs):
    """
    Initialize the random variables needed for FSSH
    """
    del kwargs
    np.random.seed(state.seed)
    state.modify("hopping_probs_rand_vals",
                 np.random.rand(len(sim.parameters.tdat)))
    state.modify(
        "stochastic_sh_rand_vals", np.random.rand(
            sim.algorithm.parameters.num_branches)
    )
    return state
...
def initialize_dm_adb_0_fssh_vectorized(sim, state, **kwargs):
    """
    Initialize the initial adiabatic density matrix in FSSH.
    """
    del sim, kwargs
    state.modify(
        "dm_adb_0",
        np.einsum("...i,...j->...ij", state.wf_adb_branch,
...
def update_act_surf_wf_vectorized(sim, state, **kwargs):
    """ 
    Update the wavefunction of the active surface
    """
    del sim, kwargs
    init_shape = np.shape(state.act_surf_ind)
    act_surf_ind_flat = state.act_surf_ind.reshape((np.prod(init_shape)))
    evecs_flat = state.eigvecs.reshape(
        (np.prod(init_shape), *np.shape(state.eigvecs)[-2:])
    )[np.arange(len(act_surf_ind_flat)), :, act_surf_ind_flat]
    act_surf_wf = evecs_flat.reshape(
        (*init_shape, np.shape(state.eigvecs)[-1]))
...
def update_dm_db_fssh_vectorized(sim, state, **kwargs):
    """
    Update the diabatic density matrix in FSSH.
    """
    del kwargs
    ...
    for nt, dm_adb_nt in enumerate(dm_adb_branch):
        np.einsum("...jj->...j", dm_adb_nt)[...] = state.act_surf[nt]
    ...
    basis_transform_mat_vectorized(
        sim,
        state,
        input_mat=dm_adb_branch,
        basis=state.eigvecs,
...
def update_wf_db_eigs_vectorized(sim, state, **kwargs):
    """
    update the diabatic wavefunctions by solving for the propagator
    in the eigenbasis.
    """
    wf_db = kwargs["wf_db"]
    ...
    basis_transform_vec_vectorized(
        sim=sim,
        state=state,
        input_vec=wf_db,
...
def initialize_timestep_index(sim, state, **kwargs):
    ...
    This function sets the timestep index (`t_ind`) in the 
    state object to an array with a single element [0].
...
def update_active_surface_fssh(sim, state, **kwargs):
    ...
    for i in range(sim.algorithm.parameters.num_branches):
        ...
        prod = np.matmul(
            np.conj(state.eigvecs[i][:, state.act_surf_ind[i]]),
            state.eigvecs_previous[i],
        )
        hop_prob = -2 * np.real(
            prod
            * (state.wf_adb_branch[i] / (state.wf_adb_branch[i][state.act_surf_ind[i]]))
        )
        ...
        for k, prob in enumerate(hop_prob):
            hop_prob[k] = nan_num(prob)
            bin_edge += hop_prob[k]
            if rand < bin_edge:
                # Perform hopping
                ...
                delta_z = dkj_zc
                # Perform hopping using the model's hop function or the default
                # harmonic oscillator hop function
                if hasattr(sim.model, "hop_function"):
                    ...
                    z_coord_branch_i, hopped = ingredients.harmonic_oscillator_hop(
                        sim.model,
                        z_coord=z_coord_branch[i],
...
def update_active_surface_fssh_vectorized(sim, state, **kwargs):
    """
    Execute the fewest-switches surface hopping (FSSH) procedure for updating the active surface.
    ...
    """
    del kwargs
    for nb in range(len(state.z_coord_branch)):
        rand = state.hopping_probs_rand_vals[nb][state.t_ind[nb][0]]
        z_coord_branch = state.z_coord_branch[nb].view()
        act_surf_ind = state.act_surf_ind[nb].view()
        act_surf = state.act_surf[nb].view()

        for i in range(sim.algorithm.parameters.num_branches):
            # Calculate hopping probabilities
            prod = np.matmul(
                np.conj(state.eigvecs[nb][i][:, state.act_surf_ind[nb][i]]),
                state.eigvecs_previous[nb][i],
            )
            hop_prob = -2 * np.real(
                prod
                * (state.wf_adb_branch[nb][i] / (state.wf_adb_branch[nb][i][state.act_surf_ind[nb][i]]))
            )
            hop_prob[state.act_surf_ind[nb][i]] = 0
            ...
            for k, prob in enumerate(hop_prob):
                ...
                if rand < bin_edge:
                    # Perform hopping
                    evec_k = state.eigvecs[nb][i][:, state.act_surf_ind[nb][i]]
                    evec_j = state.eigvecs[nb][i][:, k]
                    eval_k = state.eigvals[nb][i][state.act_surf_ind[nb][i]]
                    eval_j = state.eigvals[nb][i][k]
                    ev_diff = eval_j - eval_k
                    ...
                    dkj_z = (
                        np.einsum(
                            "...i,...nij,...j->...n",
                            np.conj(evec_k),
                            np.einsum("...nij->...nji", state.dh_qc_dzc[nb][i]).conj(),
                            evec_j,
                        )
                        / ev_diff[..., np.newaxis]
                    )
    ...
    state.modify("act_surf_ind", state.act_surf_ind)
    state.modify("act_surf", state.act_surf)
    state.modify("z_coord_branch", state.z_coord_branch)
    return state