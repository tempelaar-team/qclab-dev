...
def dynamics(model, recipe, traj=simulation.Trajectory()):
        ...
        if np.mod(t_ind, state.model.dt_output_n) == 0:
            ...
            if t_ind == 0:
                for key in observables_t.keys():
                    traj.new_observable(key, (len(state.model.tdat_output), *np.shape(observables_t[key])),
                                        observables_t[key].dtype)
            # add the variables from the dictionary observables_t to the trajectory object
            traj.add_observable_dict(int(t_ind / state.model.dt_output_n),
    ...
    traj.add_to_dic('t', state.model.tdat_output * state.model.batch_size)
    return traj
