...
def dynamics(sim,traj=simulation.Trajectory(None)):
    ...
    num_class_coords = sim.num_class_coords
    ...
    if sim.dynamics_method == 'CFSSH' or sim.dynamics_method == 'FSSH':
        ...
        for n in range(num_trajs):
            evals_0, evecs_0 = np.linalg.eigh(h_tot_branch[n*num_branches]) # first set of eigs corresponding to branches belonging to trajectory n
            z_branch_0 = z_branch[n*num_branches] # first set of coordinates corresponding to branches belonging to trajectory n
        ...
        if sim.sh_deterministic:
            ...
            for nt in range(num_trajs):
                ...
                for n in range(num_branches):
                    act_surf_ind_0[nt, n] = np.arange(num_states,dtype=int)[intervals[nt] > stochastic_sh_rand_vals[nt, n]][0]
        ...
        for nt in range(num_trajs):
            act_surf_branch[nt][np.arange(num_branches, dtype=int), act_surf_ind_branch[nt*num_branches:(nt+1)*num_branches]] = 1
        ...
        for nt in range(num_trajs):
            psi_adb_delta_branch[nt][np.arange(num_branches, dtype=int), act_surf_ind_0[nt*num_branches:(nt+1)*num_branches]] = 1.0 + 0.j
    ...
    for t_bath_ind in np.arange(0, len(tdat_bath)):
        ...
        if tdat[t_ind] <= tdat_bath[t_bath_ind] + 0.5 * sim.dt_bath:
            ...
            if sim.calc_cfssh_obs:
                if sim.cfssh_branch_pair_update == 1 and sim.dmat_const == 1:  # update branch-pairs every output timestep
                    ...
                    for nt in range(num_trajs):
                        evals_branch_pair[nt], evecs_branch_pair[nt] = auxilliary.get_branch_pair_eigs(z_branch[nt*num_branches:(nt+1)*num_branches], evecs_branch_pair_previous[nt], sim)
                # calculate overlap matrix
                overlap = np.zeros((num_trajs, num_branches, num_branches))
                for nt in range(num_trajs):
                    overlap[nt] = auxilliary.get_classical_overlap(z_branch[nt*num_branches:(nt+1)*num_branches], sim)
                if sim.dmat_const == 0:
                    ...
                    for nt in range(num_trajs):
                        ...
                        for i in range(num_branches):
                            for j in range(i+1, num_branches):
                                ...
                                if a_i != a_j and a_i != a_j and a_i == a_i_0 and a_j == a_j_0 and np.abs(rho_adb_0_traj[nt][a_i,a_j]) > 1e-12:
                                    if sim.sh_deterministic:
                                        prob_fac = 1
                                    else:
                                        prob_fac = 1/(rho_adb_0_traj[nt][a_i,a_i]*rho_adb_0_traj[nt][a_j,a_j]*(num_branches-1))
                        ...
                        if sim.sh_deterministic:
                            ...
                            rho_adb_cfssh_branch_diag = np.diag(rho_adb_0_traj[nt]).reshape((-1, 1)) * act_surf_branch_nt
                            ...
                            rho_adb_cfssh_branch[nt] = (rho_adb_cfssh_branch[nt] + rho_adb_cfssh_coh[nt] / num_branches) / num_branches
                    rho_adb_cfssh_branch = rho_adb_cfssh_branch.reshape(num_trajs*num_branches, num_states, num_states)
                ...
                if sim.dmat_const == 1:
                            ...
                            for j in range(i + 1, num_branches):
                                ...
                                if a_i != a_j and a_i != a_j and a_i == a_i_0 and a_j == a_j_0 and np.abs(rho_adb_0_traj[nt][a_i,a_j]) > 1e-12:
                                    if sim.cfssh_branch_pair_update == 0:
                                        ...
                                        evals_branch_pair[nt][i,j], evecs_branch_pair[nt][i,j] = np.linalg.eigh(h_tot_branch_ij)
                                        evecs_branch_pair_ij_tmp,_ = auxilliary.sign_adjust_branch(evecs_branch_pair[nt][i,j].reshape(1,num_states,num_states),
                                    ...
                                    if sim.sh_deterministic:
                                        ...
                                        prob_fac = 1/(rho_adb_0_traj[nt][a_i,a_i]*rho_adb_0_traj[nt][a_j,a_j]*(num_branches-1))
                                    coh_ij_tmp = prob_fac*rho_adb_0_traj[nt][a_i,a_j]*overlap[nt][i,j]*np.exp(-1.0j*(phase_branch_nt[i]-phase_branch_nt[j]))
                                    ...
                                    rho_db_cfssh_coh_ij = coh_ij_tmp*np.outer(evecs_branch_pair[nt][i,j][:,a_i],np.conj(evecs_branch_pair[nt][i,j][:,a_j])) + \
                                        np.conj(coh_ij_tmp)*np.outer(evecs_branch_pair[nt][i,j][:,a_j],np.conj(evecs_branch_pair[nt][i,j][:,a_i]))
                    ...
                    rho_adb_cfssh_branch = rho_adb_cfssh_branch.reshape((num_trajs*num_branches,num_states,num_states))
                    rho_db_cfssh_branch = auxilliary.rho_adb_to_db_branch(rho_adb_cfssh_branch, evecs_branch).reshape((num_trajs,num_branches,num_states,num_states))
                    if sim.sh_deterministic:
                        rho_db_cfssh_branch = (rho_db_cfssh_branch + (rho_db_cfssh_coh[:,np.newaxis,:,:]/num_branches))
                    else:
                        rho_db_cfssh_branch = (rho_db_cfssh_branch + (rho_db_cfssh_coh[:,np.newaxis,:,:]/num_branches))/num_branches
                    rho_db_cfssh_branch = rho_db_cfssh_branch.reshape((num_trajs*num_branches,num_states,num_states))
                    rho_db_cfssh = np.sum(rho_db_cfssh_branch, axis=0)
            ############################################################
            #                                 FSSH                     #
            ############################################################
            if sim.calc_fssh_obs:
                if sim.dmat_const == 0:
                    ...
                    for nt in range(num_trajs):
                        np.einsum('...jj->...j', rho_adb_fssh[nt*num_branches:(nt+1)*num_branches])[...] = act_surf_branch[nt*num_branches:(nt+1)*num_branches]
                    rho_db_fssh_branch = auxilliary.rho_adb_to_db_branch(rho_adb_fssh, evecs_branch)
                    if sim.sh_deterministic:
                        rho_db_fssh_branch = ((np.einsum('njj->nj',rho_adb_0_traj).reshape((num_trajs,num_states)))[:,:,np.newaxis,np.newaxis]*rho_db_fssh_branch.reshape(num_trajs, num_branches, num_states, num_states))
                    else:
        ...
        if sim.dynamics_method == 'FSSH' or sim.dynamics_method == 'CFSSH':
            ...
            for nt in range(num_trajs):
                ...
                evecs_branch[nt*num_branches:(nt+1)*num_branches], _ = auxilliary.sign_adjust_branch(evecs_branch[nt*num_branches:(nt+1)*num_branches], \
                    evecs_branch_previous[nt*num_branches:(nt+1)*num_branches], evals_branch[nt*num_branches:(nt+1)*num_branches], z_branch[nt*num_branches:(nt+1)*num_branches], sim)
            # propagate phases
            phase_branch = phase_branch + sim.dt_bath * evals_branch[np.arange(num_branches*num_trajs,dtype=int),act_surf_ind_0]
            ...
            if sim.cfssh_branch_pair_update == 2 and sim.dmat_const == 1:  # update branch-pairs every bath timestep
                evecs_branch_pair_previous = np.copy(evecs_branch_pair)
                for nt in range(num_trajs):
                    evals_branch_pair[nt], evecs_branch_pair_previous[nt] = auxilliary.get_branch_pair_eigs(z_branch[nt*num_branches:(nt+1)*num_branches], evecs_branch_pair_previous[nt], sim)
            ############################################################
            #                         HOPPING PROCEDURE                #
            ############################################################
            for nt in range(num_trajs):
                ...
                for i in range(num_branches):
                    ...
                    prod = np.matmul(np.conjugate(evecs_branch[nt*num_branches:(nt+1)*num_branches][i][:, act_surf_ind_branch[nt*num_branches:(nt+1)*num_branches][i]]),\
                    ...
                    if sim.pab_cohere:
                        hop_prob = -2 * np.real(prod * (psi_adb_branch[nt*num_branches:(nt+1)*num_branches][i] \
                                                        / psi_adb_branch[nt*num_branches:(nt+1)*num_branches][i][act_surf_ind_branch[nt*num_branches:(nt+1)*num_branches][i]]))
                    if not sim.pab_cohere:
                        hop_prob = -2 * np.real(
                            prod * (psi_adb_delta_branch[nt*num_branches:(nt+1)*num_branches][i]\
                                     / psi_adb_delta_branch[nt*num_branches:(nt+1)*num_branches][i][act_surf_ind_branch[nt*num_branches:(nt+1)*num_branches][i]]))
                    ...
                    for k in range(len(hop_prob)):
                        ...
                        if rand < bin_edge:
                            ...
                            evec_k = evecs_branch[nt*num_branches:(nt+1)*num_branches][i][:, act_surf_ind_branch[nt*num_branches:(nt+1)*num_branches][i]]
                            evec_j = evecs_branch[nt*num_branches:(nt+1)*num_branches][i][:, k]
                            eval_k = evals_branch[nt*num_branches:(nt+1)*num_branches][i][act_surf_ind_branch[nt*num_branches:(nt+1)*num_branches][i]]
                            eval_j = evals_branch[nt*num_branches:(nt+1)*num_branches][i][k]
                            ev_diff = eval_j - eval_k
                            # dkj_q is wrt q dkj_p is wrt p.
                            dkj_z, dkj_zc = auxilliary.get_dab(evec_k, evec_j, ev_diff, z_branch[nt*num_branches:(nt+1)*num_branches][i], sim)
                            ...
                            delta_z = dkj_zc
                            z_branch[nt*num_branches:(nt+1)*num_branches][i], hopped = sim.hop(sim, z_branch[nt*num_branches:(nt+1)*num_branches][i], delta_z, ev_diff)
                            if hopped: # adjust active surfaces if a hop has occured
                                act_surf_ind_branch[nt*num_branches:(nt+1)*num_branches][i] = k
                                act_surf_branch[nt*num_branches:(nt+1)*num_branches][i] = np.zeros_like(act_surf_branch[nt*num_branches:(nt+1)*num_branches][i])
                                act_surf_branch[nt*num_branches:(nt+1)*num_branches][i][act_surf_ind_branch[nt*num_branches:(nt+1)*num_branches][i]] = 1
                            break
