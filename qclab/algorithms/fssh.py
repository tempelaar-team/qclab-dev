...
class FewestSwitchesSurfaceHoppingDynamics:
    def __init__(self, sim):
        self.dm_adb_branch = None
        ...
        self.t_ind = None
        self.evecs_branch_previous = None
        self.wf_db_delta = None
        self.wf_adb_delta = None
        self.wf_db = None
        self.act_surf_branch = None
        self.act_surf_ind_branch = None
        self.act_surf_ind_0 = None
        self.h_tot = None
        self.h_q = None
        self.evecs_branch = None
        self.evals_branch = None
        self.dm_adb_0_traj = None
        self.wf_adb = None
        self.z_coord = None
        self.stochastic_sh_rand_vals = None
        self.hopping_probs_rand_vals = None
        self.tdat = None
    ...
    def initialize_dynamics(self, sim):
        ...
        for traj_n in range(num_trajs):
            evals_0, evecs_0 = np.linalg.eigh(self.h_tot[traj_n * num_branches])  # first set of eigs corresponding to
            # branches belonging to trajectory n
            z_coord_0 = self.z_coord[
                traj_n * num_branches]  # first set of coordinates corresponding to branches belonging to trajectory n
        ...
        if sim.sh_deterministic:
            ...
            for traj_n in range(num_trajs):
                for branch_n in range(num_branches):
                    act_surf_ind_0[traj_n, branch_n] = np.arange(num_states, dtype=int)[
                        intervals[traj_n] > self.stochastic_sh_rand_vals[traj_n, branch_n]][0]
        ...
        for nt in range(num_trajs):
            act_surf_branch[nt][np.arange(num_branches, dtype=int), self.act_surf_ind_branch[
                                                                    nt * num_branches:(nt + 1) * num_branches]] = 1
        ...
        for nt in range(num_trajs):
            wf_adb_delta[nt][np.arange(num_branches, dtype=int), self.act_surf_ind_0[
                                                                 nt * num_branches:(nt + 1) * num_branches]] = 1.0 + 0.j
        ...
        self.quantum_force_zc = auxilliary.quantum_force_branch(self.evecs_branch, self.act_surf_ind_branch,
                                                                self.z_coord, sim)
        return
    ...
    def calculate_observables(self, sim):
        ...
        num_branches = sim.num_branches
        if sim.dmat_const == 0:
            self.dm_adb_branch = np.einsum('ni,nj->nij', self.wf_adb, np.conj(self.wf_adb))
            for nt in range(num_trajs):
                np.einsum('...jj->...j', self.dm_adb_branch[nt * num_branches:(nt + 1) * num_branches])[
                    ...] = self.act_surf_branch[nt * num_branches:(nt + 1) * num_branches]
            self.dm_db_branch = auxilliary.rho_adb_to_db_branch(self.dm_adb_branch, self.evecs_branch)
            if sim.sh_deterministic:
                self.dm_db_branch = (
                        (np.einsum('njj->nj', self.dm_adb_0_traj).reshape((num_trajs, num_states)))[:, :,
                        np.newaxis, np.newaxis] * self.dm_db_branch.reshape(num_trajs, num_branches,
