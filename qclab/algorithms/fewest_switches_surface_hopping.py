...
class FewestSwitchesSurfaceHopping(AlgorithmClass):
    ...
    initialization_recipe = [
        ...
        lambda sim, state: tasks.broadcast_var_to_branch_vectorized(sim=sim, state=state, val=state.z_coord,
                                                                    name='z_coord_branch'),
    ...
    update_recipe = [
        ...
        lambda sim, state: tasks.copy_value_vectorized(sim=sim, state=state, val=state.eigvals,
                                                       name='eigvals_previous'),
        lambda sim, state: tasks.update_z_coord_rk4_vectorized(sim=sim, state=state, wf=state.act_surf_wf,
