...
class FewestSwitchesSurfaceHopping(Algorithm):
    ...
    def __init__(self, settings=None):
        ...
        self.update_recipe = [
            ...
            #lambda sim, parameters, state: tasks.update_wf_db_eigs_vectorized(sim=sim, parameters=parameters, state=state, wf_db=state.wf_db_branch,eigvals=state.eigvals, eigvecs=state.eigvecs,adb_name='wf_adb_branch', output_name='wf_db_branch'),
            #lambda sim, parameters, state: tasks.update_h_quantum_vectorized(sim=sim, parameters=parameters, state=state, z_coord=state.z_coord_branch),
            #lambda sim, parameters, state: tasks.diagonalize_matrix_vectorized(sim=sim, parameters=parameters, state=state, matrix=state.h_quantum,eigvals_name='eigvals', eigvecs_name='eigvecs'),
            #lambda sim, parameters, state: tasks.gauge_fix_eigs_vectorized(sim=sim, parameters=parameters, state=state, eigvals=state.eigvals,eigvecs=state.eigvecs,eigvecs_previous=state.eigvecs_previous,output_eigvecs_name='eigvecs', z_coord=state.z_coord_branch,gauge_fixing=sim.algorithm.parameters.gauge_fixing),
