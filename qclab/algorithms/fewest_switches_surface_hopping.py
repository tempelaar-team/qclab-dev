...
class FewestSwitchesSurfaceHopping(Algorithm):
    ...
    def __init__(self, settings=None):
        ...
        self.initialization_recipe = [
            ...
            lambda sim, parameters, state: tasks.broadcast_var_to_branch(sim=sim, parameters=parameters, state=state, val=state.z_coord,name='z_coord_branch'),
            lambda sim, parameters, state: tasks.broadcast_var_to_branch(sim=sim, parameters=parameters, state=state, val=state.wf_db,name='wf_db_branch'),
            ...
            lambda sim, parameters, state: tasks.diagonalize_matrix(sim=sim, parameters=parameters, state=state, matrix=state.h_quantum,eigvals_name='eigvals', eigvecs_name='eigvecs'),
            lambda sim, parameters, state: tasks.gauge_fix_eigs(sim=sim, parameters=parameters, state=state, eigvals=state.eigvals,eigvecs=state.eigvecs, eigvecs_previous=state.eigvecs,output_eigvecs_name='eigvecs', z_coord=state.z_coord_branch,gauge_fixing=2),
            lambda sim, parameters, state: tasks.copy_value(sim=sim, parameters=parameters, state=state, val=state.eigvecs,name='eigvecs_previous'),
            lambda sim, parameters, state: tasks.copy_value(sim=sim, parameters=parameters, state=state, val=state.eigvals,name='eigvals_previous'),
            lambda sim, parameters, state: tasks.basis_transform_vec(sim=sim, parameters=parameters, state=state, input_vec=state.wf_db_branch,basis=np.einsum('...ij->...ji', state.eigvecs).conj(),output_name='wf_adb_branch'),
        ...
        self.update_recipe = [
            ...
            lambda sim, parameters, state: tasks.update_h_quantum(sim=sim, parameters=parameters, state=state, z_coord=state.z_coord_branch),
            lambda sim, parameters, state: tasks.diagonalize_matrix(sim=sim, parameters=parameters, state=state, matrix=state.h_quantum,eigvals_name='eigvals', eigvecs_name='eigvecs'),
            lambda sim, parameters, state: tasks.gauge_fix_eigs(sim=sim, parameters=parameters, state=state, eigvals=state.eigvals,eigvecs=state.eigvecs,eigvecs_previous=state.eigvecs_previous,output_eigvecs_name='eigvecs', z_coord=state.z_coord_branch,gauge_fixing=sim.algorithm.settings.gauge_fixing),
