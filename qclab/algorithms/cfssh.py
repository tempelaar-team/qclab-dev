...
class CoherentFewestSwitchesSurfaceHoppingDynamics:
    def __init__(self, sim):
        ...
        self.t_ind = None
        self.evecs_branch_pair_previous = None
        ...
        self.h_tot = None
        self.h_q = None
        self.evals_branch_pair = None
        self.evecs_branch_pair = None
    ...
    def initialize_dynamics(self, sim):
        ...
        if sim.dmat_const > 0:
            self.evecs_branch_pair = np.zeros((num_trajs, num_branches, num_branches, num_states, num_states),
                                              dtype=complex)
        ...
        for traj_n in range(num_trajs):
            evals_0, evecs_0 = np.linalg.eigh(self.h_tot[
                                                  traj_n * num_branches])  # first set of eigs corresponding to
        ...
        if sim.dmat_const == 0:
            ...
            for nt in range(num_trajs):
                ...
                if sim.sh_deterministic:
                    ...
                    self.dm_adb[nt] = (self.dm_adb[nt] + dm_adb_coh[nt] / num_branches) / num_branches
                    ...
                    for j in range(i + 1, num_branches):
                        ...
                        if a_i != a_j and a_i != a_j and a_i == a_i_0 and a_j == a_j_0 and np.abs(
                            ...
                            if sim.cfssh_branch_pair_update == 0:
                                ...
                                self.evals_branch_pair[nt][i, j], self.evecs_branch_pair[nt][i, j] = np.linalg.eigh(
                                    h_tot_branch_ij)
                            ...
                            dm_db_coh_ij = coh_ij_tmp * np.outer(self.evecs_branch_pair[nt][i, j][:, a_i],
                                           ...
                                           np.conj(coh_ij_tmp) * np.outer(self.evecs_branch_pair[nt][i, j][:, a_j],
                                                                          np.conj(
                                                                              self.evecs_branch_pair[nt][i, j][:, a_i]))
