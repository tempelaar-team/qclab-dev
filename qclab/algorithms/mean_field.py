...
class MeanField(Algorithm):
    ...
    initialization_recipe = [
        ...
        lambda sim, state: tasks.update_h_quantum_vectorized(
            sim=sim, state=state, z_coord=state.z_coord),
    ]
    update_recipe = [
        lambda sim, state: tasks.update_h_quantum_vectorized(
            sim=sim, state=state, z_coord=state.z_coord),
    ...
    output_recipe = [
        ...
        lambda sim, state: tasks.update_quantum_energy_mf_vectorized(
            sim=sim, state=state, wf=state.wf_db),
        lambda sim, state: tasks.update_classical_energy_vectorized(
            sim=sim, state=state, z_coord=state.z_coord),
    ]
    output_variables = [
        'dm_db',
