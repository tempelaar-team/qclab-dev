...
class MeanField(AlgorithmClass):
    ...
    The algorithm class has a set of parameters that define the algorithm
    Some of these parameters depends on the model i.e. num_branches is always the same as the number of quantum states in the model
    for deterministic surface hopping methods. 
    ...
    update_recipe = [
        ...
        lambda sim, state: tasks.update_z_coord_rk4_vectorized(sim = sim, state = state,z_coord = state.z_coord, output_name='z_coord', wf = state.wf_db, update_quantum_classical_forces_bool=False),
