...
def update_hq(q, p):
    ...
    for j in range(ndyn_state):
        ...
        qn = (np.real(phrot_d).dot(qmat)
              - np.imag(phrot_d).dot(pmat / frq)).dot(coup_const)  # vector with index running over unrotated basis n
...
def get_fssh_phonon_force(active_vec):
    """
    ...
    dhdq = np.outer(re_cvcv, coup_const)  # matrix with the order of indices being alp x i
    dhdp = np.outer(im_cvcv, -(coup_const / frq))  # matrix with the order of indices being alp x i
...
def get_mf_phonon_force(coef):
    ...
    dhdq = np.outer(re_ccvv, coup_const)  # matrix with the order of indices being alp x i
    dhdp = np.outer(im_ccvv, -(coup_const / frq))  # matrix with the order of indices being alp x i
...
def get_nac_numer(eigvec):
    ...
    for gam in range(ndyn_state):
        for i in range(nph_per_set):
            nacq_num[nph_per_set * gam + i] = re_cvvc[gam] * coup_const[i]
            nacp_num[nph_per_set * gam + i] = im_cvvc[gam] * (-coup_const[i] / frq[i])
