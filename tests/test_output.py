...
from qc_lab.models import SpinBoson, HolsteinLattice, FMOComplex, TullyProblemOne, TullyProblemTwo, TullyProblemThree
...
def test_output():
    ...
    sim = Simulation({"num_trajs":10,"batch_size":10,"tmax":10,"dt_update":0.01,"dt_collect":0.1,"progress_bar":False,})
    ...
    for model_class in [SpinBoson, HolsteinLattice, FMOComplex, TullyProblemOne, TullyProblemTwo, TullyProblemThree]:
        for algorithm_class in [MeanField, FewestSwitchesSurfaceHopping]:
            ...
            data_correct = Data().load(os.path.join(reference_folder, f"{model_name}_{algorithm_name}.h5"))
            for key, val in data.data_dict.items():
                np.testing.assert_allclose(val, data_correct.data_dict[key], rtol=1e-5, atol=1e-8)
    return
